...
if "%FILE_TO_VERIFY%"=="" (
    echo %SYM_ERROR% File path not provided for verification.
    exit /b 1
)

if "%EXPECTED_SIZE%"=="" (
    echo %SYM_ERROR% Expected file size not provided.
    exit /b 1
)

if "%EXPECTED_HASH%"=="" (
    echo %SYM_ERROR% Expected SHA256 hash not provided.
    exit /b 1
)

if not exist "%FILE_TO_VERIFY%" (
    echo %SYM_ERROR% File to verify does not exist: %FILE_TO_VERIFY%
    exit /b 1
)
...
if "!ACTUAL_SIZE!" neq "%EXPECTED_SIZE%" (
    echo %SYM_ERROR% File size mismatch! Expected: %EXPECTED_SIZE%, Actual: !ACTUAL_SIZE!
    exit /b 2
)
...
if "!ACTUAL_HASH!" neq "%EXPECTED_HASH%" (
    echo %SYM_ERROR% SHA256 hash mismatch! 
