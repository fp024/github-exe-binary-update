...
if not exist "%DEFENDER_CMD%" (
    echo %SYM_INFO% Windows Defender command line tool not found.
...
echo %SYM_DEBUG% Executing "%DEFENDER_CMD%" -Scan -ScanType 3 -File "%ABSOLUTE_PATH%"
"%DEFENDER_CMD%" -Scan -ScanType 3 -File "%ABSOLUTE_PATH%" > scan_output.tmp 2>&1
set CMD_EXIT_CODE=%errorlevel%
echo %SYM_DEBUG% MpCmdRun.exe returned exit code: %CMD_EXIT_CODE%
...
if !errorlevel! equ 0 (
    echo %SYM_DEBUG% No threats found in output text
    set THREAT_DETECTED=0
) else (
    findstr /R "found [0-9]+ threats" scan_output.tmp > nul
    if !errorlevel! equ 0 (
        echo %SYM_DEBUG% Threats detected in output text
        set THREAT_DETECTED=1
    ) else (
        echo %SYM_DEBUG% Could not determine threat status from output
        set THREAT_DETECTED=0
    )
)
